...
int main(int argc, char **argv)
{
    ...
    // If the given address is not an IP address, resolve it using DNS
    if (ipaddr == -1 ) {
      // Code to get the host address and convert it to an IP address
    ...
    for(;;) {
      ...
      if ( FD_ISSET(STDIN, &read_fds) ) {
               nbytes = read(STDIN, buf,sizeof(buf)-1);
